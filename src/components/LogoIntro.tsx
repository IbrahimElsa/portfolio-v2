...
export default function IntroWrapper({ children }: { children: React.ReactNode }) {
  ...
  const [animationStep, setAnimationStep] = useState(1); // 1: intro, 2: moving, 3: final
  
  useEffect(() => {
    // Step 1: Assembly animation
    const step2Timer = setTimeout(() => {
      setAnimationStep(2); // Start moving to corner
    }, 3000);
    
    // Step 2: After movement to corner, show content
      ...
      >
        ...
        >
          ...
          <motion.path
            ...
            transition={{ 
              duration: animationStep === 1 ? 2.8 : 0,
              times: animationStep === 1 ? [0, 0.5, 1] : [0, 1],
              ease: "easeOut"
            }}
          />
          ...
          <motion.path
            ...
            transition={{ 
              ...
              times: animationStep === 1 ? [0, 0.5, 1] : [0, 1],
              ease: "easeOut",
              delay: animationStep === 1 ? 0.3 : 0
            }}
          />
        </svg>
      </motion.div>
      
      {/* Background overlay - fades out after logo moves to corner */}
      <motion.div 
